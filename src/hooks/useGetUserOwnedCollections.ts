    ...
    const fetchCollectionInfos = async () => {
      ...
      try {
        const allCollectionInfo = await Promise.all(
          uniqueCollectionIds.map(async (collection_id) => {
            try {
              const fieldsData = await suiClient.getDynamicFields({ parentId: collection_id });
              const objectIds = fieldsData.data.map((item: any) => item.objectId);
              const allObjectIds = [collection_id, ...objectIds];
